...
void Motor_Init(void)
{
	...
	HAL_GPIO_WritePin(Sprayer_GPIO_Port, Sprayer_Pin, GPIO_PIN_RESET);	//低电平
...
static void MOTOR1_SetSpeed(int16_t speed)
{
	...
	if(speed > 0)
	{
		HAL_GPIO_WritePin(M1IN1_GPIO_Port, M1IN1_Pin, GPIO_PIN_SET);		//高电平
		HAL_GPIO_WritePin(M1IN2_GPIO_Port, M1IN2_Pin, GPIO_PIN_RESET);	//低电平
	...
	__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_1, temp);   //temp为占空比
}
