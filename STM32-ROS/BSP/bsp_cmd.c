...
#define SCB_AIRCR       (*(volatile unsigned long *)0xE000ED0C)  /* Reset control Address Register 复位控制地址寄存器 */
#define SCB_RESET_VALUE 0x05FA0004                               /* Reset value, write to SCB_AIRCR can reset cpu 复位值，写入SCB_AIRCR可以复位cpu*/
...
void cmd(void)
{
	...
	else if(!strcmp(CMD_Buffer,"show_encode"))       {encoder_value();}
	else
	{
...
void help(void)
{
	...
	printf("show_encode          ----------     Show encode \r\n");
...
void encoder_value(void)
{
	extern int16_t encoder_delta[4];
	while(1)
	{
		printf("%d\r\n", encoder_delta[0]);   //编码器变化值
