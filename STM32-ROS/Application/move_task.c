...
void move_task(void const * argument)
{
	...
	{
		//获取编码器变化值
		encoder_delta[0] = -(Encoder_Get_Counter(1) - ENCODER_MID_VALUE);  //设置编码器初始中间值
		...
		HAL_GPIO_WritePin(Sprayer_GPIO_Port, Sprayer_Pin, GPIO_PIN_SET);}//点亮
		if(robot_target_speed[0] < 0){
		HAL_GPIO_WritePin(Sprayer_GPIO_Port, Sprayer_Pin, GPIO_PIN_SET);}
		if(robot_target_speed[0] ==  0){
		HAL_GPIO_WritePin(Sprayer_GPIO_Port, Sprayer_Pin, GPIO_PIN_RESET);}
